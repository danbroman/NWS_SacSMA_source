...
C
      IDT=PLOSS(10)
C     IBEGIN=(IDA-IDADAT)*24/IDT+IHR/IDT
      IBEGIN=1
C     IEND=(LDA-IDADAT)*24/IDT+LHR/IDT
      IEND=(LDA-IDADAT)*24/IDT+(LHR-IHR)/IDT +1

C.......................................................................
C     DEBUG INFORMATION
CVR   IF(IBUG.EQ.1) WRITE(IODBUG,913) (CLOSS(1))
C 913 FORMAT(1H0,18HCLOSS VALUE       ,(T30,F10.5))
CVR   IF(IBUG.EQ.1) WRITE(IODBUG,912) (PE(I),I=IBEGIN,IEND)
C 912 FORMAT(1H0,18HPOTEN   EVAPOR    ,(T30,10F10.5))
CVR   IF(IBUG.EQ.1) WRITE(IODBUG,910) (Q(I),I=IBEGIN,IEND)
CV910 FORMAT(1H0,18HINITIAL DISCHARGES,(T30,10F10.1))
      
      IF(FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING, *)'CLOSS VALUE ',CLOSS(1)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING, *) 'POTEN EVAPOR'
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        WRITE(FORMATSTR,*) '%10.5f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     +  1, 10, FORMATSTR, IBEGIN, IEND, 1, PE)

        WRITE(MESSAGESTRING, *) 'INITIAL DISCHARGES'
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        WRITE(FORMATSTR,*) '%10.1f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     +  1, 10, FORMATSTR, IBEGIN, IEND, 1, Q)
      ENDIF
...
C
C     DEBUG INFORMATION
CVR75 IF(IBUG.EQ.1) WRITE(IODBUG,920) (Q(I),I=IBEGIN,IEND)
CV920 FORMAT(1H0,27HDISCHARGES AFTER SSOUT LOSS,(T30,10F10.1))

   75 IF(FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING, *)'DISCHARGES AFTER SSOUT LOSS'
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        WRITE(FORMATSTR,*) '%10.1f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     +  1, 10, FORMATSTR, IBEGIN, IEND, 1, Q)
...
C     UPDATE CARRYOVER IF REQUESTED
 ...
 161  PECO=PE(IP)
 162  NCO=1
CVR   CALL FCWTCO(MDA,MHR,PECO,NCO)
...
C.......................................................................
 170  CONTINUE
C     DEBUG INFORMATION
CVR   IF(IBUG.EQ.1) WRITE(IODBUG,930) (Q(I),I=IBEGIN,IEND)
CV930 FORMAT(1H0,28HDISCH. AFTER SSOUT/EVAP LOSS,(T30,10F10.1))

      IF(FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING, *)'DISCH. AFTER SSOUT/EVAP LOSS'
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        WRITE(FORMATSTR,*) '%10.1f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     +  1, 10, FORMATSTR, IBEGIN, IEND, 1, Q)
      WRITE(MESSAGESTRING, *)'AT END CLOSS(1)=', CLOSS(1)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      ENDIF

      IF(ITRACE.GE.1) THEN
        WRITE(MESSAGESTRING,800)
 800    FORMAT('EX8 EXITED')
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      ENDIF
