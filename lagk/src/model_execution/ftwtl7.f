...
C
C
CFC      IF (IBUG.EQ.1) WRITE (IODBUG,600) TLRC,QBNTL,QLI,QPREV,QK
      IF(FEWSDEBUG .GE. 1) THEN
      	WRITE(MESSAGESTRING,600) TLRC,QBNTL,QLI,QPREV,QK
...
C
      Q=QTL
C
CFC 999  IF (IBUG.GT.0) WRITE (IODBUG,601) Q,QTL
 999  IF(FEWSDEBUG .GE. 1) THEN
      	WRITE(MESSAGESTRING,601) Q,QTL
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF 
