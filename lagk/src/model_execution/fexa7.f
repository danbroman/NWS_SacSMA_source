...
C
C
CFC      IF(IB.EQ.1)WRITE(IODBUG,*) 'ENTER FEXA7'
      IF(FEWSDEBUG .GE. 1) THEN
      	WRITE(MESSAGESTRING,*) 'ENTER FEXA7'
...
C
      ITA=P(5)
C lc update the following code; the commented out lines assume CO is not saved at end of run
C lc in CHPS CO always saved at end of run; old code caused it to run 2x for whole period
      NTIMES=1
C      IF(IFILLC.GT.0)NTIMES=NCSTOR+1
      IF(IFILLC.GT.0)NTIMES=NCSTOR
C lc the line below is because ressngl uses this code with ncstor=0 - they don't save co      
...
CFC          CALL FCWTCO(ICDAY(IIX),ICHOUR(IIX),CT,NC)
        ENDIF
C
 999  CONTINUE
...
C
 1000 CONTINUE
C
CFC      IF(IB.EQ.1)WRITE(IODBUG,*) 'EXIT FEXA7'
      IF(FEWSDEBUG .GE. 1) THEN
      	WRITE(MESSAGESTRING,*) 'EXIT FEXA7'
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF      
C
      RETURN
