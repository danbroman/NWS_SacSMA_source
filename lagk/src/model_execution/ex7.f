...
C
      NP=P(16)
      NC=C(1)
      IF(FEWSDEBUG.GE.1)CALL FPRPC7(NP,P,NC,C)
...
C
      ITA=P(5)
CFC      LDATA=(IDA-IDADAT)*24/ITA + (IHR-1)/ITA + 1
      LDATA = 1
...
C      IBUG = 1
C      IODBUG = 6
C      WRITE(*,*) 'QB(LQB) = ', QB(LQB), LQB
C      WRITE(*,*) 'QB(LCT) = ', QB(LCT), LCT
C      WRITE(*,*) 'QB(LQT) = ', QB(LQT), LQT
C      WRITE(*,*) 'LEND = ',LEND
...
C LC 
      ...
      IF(FEWSDEBUG.GE.1)THEN
          WRITE(MESSAGESTRING,901)(QB(I),I=1,MAXDEBUG)
