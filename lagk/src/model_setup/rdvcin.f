...
CFC      IF (IBUG.GT.0) WRITE (IODBUG,*) 'ENTER RDVCIN'
      IF(FEWSDEBUG .GE. 5) THEN
      	WRITE(MESSAGESTRING,*) 'ENTER RDVCIN'
...
CFC      IF (IBUG.GT.0) WRITE (IODBUG,'('' CARD='',72A1)') BB
      IF(FEWSDEBUG .GE. 5) THEN
      	WRITE(MESSAGESTRING,'('' CARD='',72A1)') BB
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF       
...
C  Read continuation card
         ...
         ELSE
           DO 50 J=1,10
             ...
             ENDIF
50         CONTINUE
CFC           WRITE (IPR,60) BB(I),I,BB
       	   WRITE(MESSAGESTRING,60) BB(I),I,BB
