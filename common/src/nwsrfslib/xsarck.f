...
C
C
!CP    IF (ITRACE.GE.1) WRITE(IODBUG,601)
!CP    601  FORMAT(1H0,20H *** ENTER XSARCK **)
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '  *** ENTER XSARCK ***'
...
C  PRINT DBUG INFO ON MANNINGS N
      STG=XRC(LH1-1+I)
!CP   IF(IBUG.GE.1)
!CP   &WRITE(IODBUG,6059) I,STG,QRC,BRC,ARC,CKARAY(I),CMN
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 6059) I,STG,QRC,BRC,ARC,CKARAY(I),CMN
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C  MAINTAIN CONSTANT CONVEYANCE ABOVE RATING CURVE
 ...
 113  CONTINUE
!CP   IF(IBUG.GE.1) WRITE(IODBUG,6060) 
!CP   & I,XRC(LXE1-1+I),XSTG,XTOPW,XAREA(I),CMANGN(I)
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 6060) 
...
C
C
!CP   IF (ITRACE.GE.1) WRITE(IODBUG,602)
!CP   602  FORMAT(1H0,20H *** EXIT XSARCK ***)
C
!     OPNAME(1) = OLDSUB(1)
!     OPNAME(2) = OLDSUB(2)
!     IOPNUM = IOLDOP
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XSARCK ***'
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
