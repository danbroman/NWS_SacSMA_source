...
C
      ...
      Y1=YVECT(N1)
      Y2=YVECT(N2)
!CP   IF(IBUG.GE.1) WRITE(IODBUG,603) XX,X1,X2,Y1,Y2
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 603) XX,X1,X2,Y1,Y2
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   IF(IBUG.GE.1) WRITE(IODBUG,607)X1,X2,Y1,Y2,XX,DYDX,
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 607) X1,X2,Y1,Y2,XX,DYDX,YY
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
  ...
  130 CONTINUE
C
!CP   IF(IBUG.GE.1) WRITE(IODBUG,604) YY
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 604) YY
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 604     FORMAT(1H0,10X,24H EXTRAPOLTED YY VALUE = ,G15.7)
      END IF
C
C
!CP   IF(ITRACE.GE.2) WRITE(IODBUG,602)
!     OPNAME(1) = OLDSUB(1)
!     OPNAME(2) = OLDSUB(2)
!     IOPNUM = IOLDOP
      IF ( FEWSDEBUG.GE.5 ) THEN
