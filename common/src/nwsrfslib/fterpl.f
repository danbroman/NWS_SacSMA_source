...
C
      XX=XXO
!CP - common/where NOT used by FEWS module
!    DO 500 I=1,2
!    OLDSUB(I) = OPNAME(I)
...
C
!CP  IF(IBUG.GE.1) WRITE(IODBUG,603) XX,XVECT(1),XVECT(N),YVECT(1),
!CP     1YVECT(N)
  ...
  601    FORMAT(1H ,21H *** ENTER FTERPL ***)
      END IF
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 603) XX,XVECT(1),XVECT(N),YVECT(1),
     $         YVECT(N)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
c jgg      IF(X1.GT.-0.0001.AND.X1.LT.0) X1=-0.0001
C jgg end

cCP r24-37 bug fixed produces NaN or -Inf values if X1 is negative value 
cCP or equals zero (when run using INTERP LOG and LAG/K).  Need to reuse
cCP the original code before r24-37

      IF(X1.LT.0.0001) X1=0.0001
...
cCP 4/21/09 reuse the original logic before bug r22-26.  Bug r22-26 
cCP removed "IF(Y1.LT.0.0001) Y1=0.0001" line because of that
cCP result produces NaN value(s) if Y1 is negative value or equals zero.
      IF(Y1.LT.0.0001) Y1=0.0001
C      
      IF(XX.LT.X1) XX=X1
...
C  THE FOLLOWING CHANGE MADE ON 02/05/92 -- MAIN. #0673
      DYDX=(Y2-Y1)/(X2-X1)
      YLIN=(XX-X1)*DYDX+Y1
!CP   IF(IBUG.GE.1) WRITE(IODBUG,607)X1,X2,Y1,Y2,XX,DYDX,
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 607) X1,X2,Y1,Y2,XX,DYDX,YLIN
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C  THE FOLLOWING CHANGE MADE ON 02/05/92 -- MAIN. #0673
      DYDX=(Y2-Y1)/(X2-X1)
      YLOG=(XX-X1)*DYDX+Y1
!CP   830 IF(IBUG.GE.1) WRITE(IODBUG,607)X1,X2,Y1,Y2,XX,DYDX,
  830 IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 607)X1,X2,Y1,Y2,XX,DYDX,YLOG
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C  END OF CHANGE OF 02/05/92
...
C
C
!CP   600 IF(IBUG.GE.1) WRITE(IODBUG,6040) YY
  600 IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 6040) YY
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 6040 FORMAT(1H0,10X,24H INTERPOLTED YY VALUE = ,G15.7)
      END IF
C
!CP   IF(ITRACE.GE.2) WRITE(IODBUG,604)
!     OPNAME(1) = OLDSUB(1)
!     OPNAME(2) = OLDSUB(2)
!     IOPNUM = IOLDOP
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 604)
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
