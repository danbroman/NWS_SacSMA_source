...
C
C  LINE PRINTER, CARD READER, DEBUG, ERROR AND CARD PUNCH UNIT
C  COMMON BLOCK
...
C
C
C      IF (IUTLTR.GE.1) WRITE (IOGDB,*) 'ENTER UNPAKS'
C
      IF (FEWSDEBUG.GE.6) THEN
          WRITE (MESSAGESTRING,*) 'ENTER UNPAKS'
...
C
      NUMS=NUM*4
      IF (NUMS.GT.MAX) THEN
          WRITE (MESSAGESTRING,40) NUMS,MAX
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
C         WRITE (LP,40) NUMS,MAX
...
50     IF (FEWSDEBUG.GE.6) THEN
          WRITE (MESSAGESTRING,*) 'EXIT UNPAKS'
          call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      ENDIF
C50    IF (IUTLTR.GE.1) WRITE (IOGDB,*) 'EXIT UNPAKS'
C
      RETURN
C
      END
